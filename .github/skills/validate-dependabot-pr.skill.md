...
description: |
  Validates open Dependabot npm PRs in chef/vscode-chef against Harness Artifact
  Registry (HAR) compliance, reviews the actual dependency change, confirms the
  `ci` GitHub workflow (validate + build) is green, and verifies the extension
  still packages and installs correctly. Produces a per-PR merge-readiness report.
  Never approves, comments on, or merges PRs — a human always merges manually.
instructions: |
